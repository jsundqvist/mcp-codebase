...
import { conditionalPattern, loopPattern, statementPattern } from '../../../src/parsers/javascript.js';
import { individual, query } from '../test-utils.js';

const runConditionals = function(parser) {
...
const runLoops = function(parser) {
	describe('Loops', () => {
...
const runStatements = function(parser) {
	describe('Statements', () => {
...
if (individual(import.meta.url)) {
	const parserConditionals = createJavaScriptParser(conditionalPattern);
	runConditionals(parserConditionals);
	const parserLoops = createJavaScriptParser(loopPattern);
	runLoops(parserLoops);
	const parserStatements = createJavaScriptParser(statementPattern);
	runStatements(parserStatements);
}

export default function(parser) {
	runConditionals(parser);
	runLoops(parser);
	runStatements(parser);
}
